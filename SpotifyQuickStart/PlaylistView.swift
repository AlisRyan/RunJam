    ...
    self.pedometer.startUpdates(from: Date()) { pedometerData, error in
        ...
        if let cadence = pedometerData?.currentCadence {
          if first {
            ...
            dispatchGroup.notify(queue: .main) {
              if let selectedTrack = self.selectTrackWithTempoInRange(from: self.tracks, targetTempo: Float(Double(truncating: cadence))) {
            ...
            print("Error fetching cadence:", error.localizedDescription)
        }
    }
  }
...
//          print(pedometerData?.currentCadence ?? "HEY")
//          if let cadence = pedometerData?.currentCadence {
//              self.selectAndPlayTrackWithClosestTempoTo(Double(truncating: cadence))
//          } else if let error = error {
//              print("Error fetching cadence:", error.localizedDescription)
//          }
//      }
//  }
  ...
  func selectAndPlayTrackWithClosestTempoTo(_ cadence: Double) {
      let tempoRange = Float(cadence * 60) // Convert cadence to tempo (BPM)
    print(tempoRange)

      if let selectedTrack = self.selectTrackWithTempoInRange(from: self.tracks, targetTempo: tempoRange) {
          print("Selected track with tempo:", selectedTrack.audioFeatures?.tempo ?? "N/A")
          spotifyController.playTrack(uri: selectedTrack.uri)
      } else {
          print("No track found with suitable tempo.")
